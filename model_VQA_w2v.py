...
class Answer_Generator():
    def __init__(self, rnn_size, rnn_layer, batch_size, input_embedding_size, dim_image, dim_hidden, max_words_q, drop_out_rate):
    ...
    def build_model(self):
        ...
        question = tf.placeholder(tf.float32, [self.batch_size, self.max_words_q, self.input_embedding_size])
        ...
        for i in range(max_words_q):
            ...
            output, state = self.stacked_lstm(question[:, i], state)
...
# Data input setting
input_img_h5 = './data_img_fc7.h5'
input_ques_h5 = './data_prepro_w2v.h5'
input_json = './data_prepro_w2v.json'
...
# Check point
checkpoint_path = 'model_save_w2v/'
...
def right_align(seq,lengths):
    ...
    for i in range(np.shape(seq)[0]):
        v[i][N-lengths[i]:] = seq[i][:lengths[i]]
    return v
...
def get_data():
    ...
    if img_norm:
        ...
        img_feature = np.divide(img_feature, np.transpose(np.tile(tem,(dim_image,1))))
...
def train():
    ...
    for itr in range(max_itr):
        ...
        sess.run(lr.assign(current_learning_rate))
...
def test(model_path='model_save_w2v/model-15000'):
    print 'loading dataset...'
    ...
    for current_batch_start_idx in xrange(0,num_test-1,batch_size):
        ...
        if(len(current_img)<500):
                ...
                pad_q = np.zeros((500-len(current_img),max_words_q, input_embedding_size),dtype=np.float)
    ...
    print 'Saving result...'
    my_list = list(result)
    dd = json.dump(my_list,open('data_w2v.json','w'))
